...
# 读取清洗后的processedTrainData.tsv文件
...
def main():
    ...
    if df is not None:
        ...
        save_model(model1, "word2vec_model_400d_cleaned")
        
        # 模型2: 增加窗口大小到15
        print("\n=== 训练模型2: 向量维度300, 窗口大小15 ===")
        model2 = train_word2vec(sentences, vector_size=300, window=15)
        save_model(model2, "word2vec_model_15window_cleaned")
        
        # 模型3: 同时调整向量维度和窗口大小
        print("\n=== 训练模型3: 向量维度400, 窗口大小15 ===")
        model3 = train_word2vec(sentences, vector_size=400, window=15)
        save_model(model3, "word2vec_model_400d_15window_cleaned")
        
        # 写入实验日志
        print("\n正在写入实验日志...")
        log_data = {
            'id': [33],
            '主要改动': ['使用清洗后的processedTrainData.tsv文件重新训练Word2Vec模型，调整参数（向量维度400，窗口大小15）'],
            '实现结果': ['成功训练了三个不同参数的Word2Vec模型，分别保存为word2vec_model_400d_cleaned.model、word2vec_model_15window_cleaned.model和word2vec_model_400d_15window_cleaned.model']
        }
