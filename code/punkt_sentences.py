...
def main():
    ...
    if df is not None:
        ...
        log_data = {
            'id': [24],
            '主要改动': ['使用正则表达式对processedTrainData.tsv文件进行句子拆分'],
            '实现结果': [f'成功读取processedTrainData.tsv文件，处理所有{total_reviews}条评论，拆分出{len(all_sentences)}个句子']
        }
